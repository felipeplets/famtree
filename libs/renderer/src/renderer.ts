...
class GenogramRenderer {
  ...
  private drawChildBus(svg: Svg, graph: GenogramGraph, pos: Positions, unionId: string, mx: number, y: number): void {
  ...
  private drawRoutedBonds(svg: Svg, graph: GenogramGraph, pos: Positions, ties: EmotionalRelationship[]): void {
    const LANE_STEP = 15
  ...
  private maxLabelDepth(graph: GenogramGraph, pos: Positions, gen: number, lo = -Infinity, hi = Infinity): number {
    let max = R + 15
  ...
  private drawBondArrow(
    ...
    const ax = tx - Math.cos(ang) * (R + 4),
      ay = ty - Math.sin(ang) * (R + 4)
    const d =
  ...
  private drawProbandMarker(svg: Svg, p: Person, x: number, y: number, stroke: string): void {
    ...
    const tipX = x - tipR * d,
      tipY = y - tipR * d // top-left point just outside the symbol
    const len = 16,
      hl = 6,
      ang = Math.PI / 4 // shaft length, head length, 45° pointing in
    const tailX = tipX - d * len,
      tailY = tipY - d * len
    const h1x = tipX - Math.cos(ang - 0.5) * hl,
      h1y = tipY - Math.sin(ang - 0.5) * hl
    const h2x = tipX - Math.cos(ang + 0.5) * hl,
      h2y = tipY - Math.sin(ang + 0.5) * hl
...
export { GenogramRenderer }
export type { RenderStats, RenderResult }
