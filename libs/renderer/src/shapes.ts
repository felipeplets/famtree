...
const female: NodeShape = {
  ...
  probandOutline: (svg, x, y, r, stroke) => svg.circle(x, y, r + 4, `fill="none" stroke="${stroke}" stroke-width="1"`),
}

const unknown: NodeShape = {
  base: (svg, x, y, r, attr) => svg.polygon(`${x},${y - r} ${x + r},${y} ${x},${y + r} ${x - r},${y}`, attr),
}

const SHAPES: Record<Sex, NodeShape> = { male, female, unknown }

function shapeFor(sex: Sex): NodeShape {
